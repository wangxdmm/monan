...
describe('resutful', async () => {
  ...
  it('create config and back is ok', () => {
    const api = http.create<[defineAPI<'get', { name: string }, { name: string }>]>('.', [
      'get::/->get',
    ])
    ...
    return new Promise((resolve) => {
      api.get({
        name: 'Jack',
      })({ notificationDelay: true }).then(({ backData, result }) => {
  ...
  it('use inputData as params in request is ok', () => {
    const api = http.create<[defineAPI<'del', { code: number }, { success: boolean }>]>('.', [
      'delete::/del?',
    ])
    const serveResponse = {
      success: true,
