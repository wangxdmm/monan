...
var _SetupAxios = class {
  constructor(config) {
    ...
    __publicField(this, "dynamicRequestConfig", /* @__PURE__ */ new Map());
    __publicField(this, "dynamicHanlder", /* @__PURE__ */ new Map());
  ...
  async catchError(error) {
    ...
    for (let i = 0; i < codeHandler.length; i++) {
      ...
      if (!on || isArray(on) && on.includes(code) || isRegExp(on) && on.test(String(code))) {
        if (isRegExp(on)) {
          on.lastIndex = 0;
        }
        const params = {
...
// 解析 path 中的参数选项
__publicField(SetupAxios, "interParam", function(urlIn, dataIn, reserve = false) {
  let url = urlIn;
...
var Restful = class extends SetupAxios {
  constructor() {
    super(...arguments);
    __publicField(this, "genHandleFunc");
    __publicField(this, "defaultStrategies", {});
  }
