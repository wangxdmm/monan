import { get, set, noop } from "../helpers";
import { SetupAxios } from "./setupAxios";
import type { Method } from "axios";
import type { defineAPI, ExtractAPI, GenHandleFunc, LabelDef, Config, DefaultStrategies } from "./share";
...
export class Restful<T> extends SetupAxios<T> {
  ...
  createDefaultStrategies<D>(strategiesCreator: (ins: this) => DefaultStrategies<D>) {
  ...
  patchConfigByMeta(userInputData, config: Config, meta: LabelDef["meta"]): Config {
    ...
    if (meta) {
      ...
      if (contentType && !get(configed, ["headers", ContentTypeKey])) {
        ContentTypeEnum[contentType] && set(configed, ["headers", ContentTypeKey], ContentTypeEnum[contentType]);
      }
  ...
  create<T extends defineAPI<string, any, any>[]>(prefix: string, defs: string[]) {
    const result = {};
    defs.forEach((def) => {
